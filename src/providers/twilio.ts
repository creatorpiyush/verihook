import { computeHmacSha1, computeSha256, timingSafeEqual } from '../core/crypto.js';
  ...
  async verify(
    ...
    if (!url) {
      return {
        ...
        reason: 'Missing request URL. Provide request URL or pass options.url explicitly for Twilio verification',
      };
    }
    ...
    const hasExplicitFormContentType = contentType.includes('application/x-www-form-urlencoded');
    const hasMissingContentType = !contentType;
    const looksLikeFormBody = /(^|&)[^=&]+=[^&]*/.test(req.rawBody);
    const isFormUrlEncoded = hasExplicitFormContentType || (hasMissingContentType && looksLikeFormBody);
    ...
    if (isFormUrlEncoded && req.rawBody) {
      ...
      try {
        ...
        if (bodyShaRegex.test(url)) {
          dataToSign = url.replace(bodyShaRegex, `$1bodySHA256=${encodeURIComponent(bodyHashHex)}`);
        } else {

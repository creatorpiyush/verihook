...
export const metaVerifier: ProviderVerifier = {
  ...
  async verify(
    ...
    if (sig256) {
      const cleanSig256 = sig256.startsWith('sha256=') ? sig256.slice(7) : sig256;
      const hmac256 = await computeHmacSha256(secret, req.rawBody);
      const expected256 = bytesToHex(hmac256);
      if (timingSafeEqual(cleanSig256.trim().toLowerCase(), expected256.toLowerCase())) {
        return {
          valid: true,
          provider: 'meta',
        };
    ...
    if (sig1) {
      ...
      if (timingSafeEqual(cleanSig1.trim().toLowerCase(), expected1.toLowerCase())) {
        return {
          valid: true,
          provider: 'meta',
        };
...
export function verifyMetaChallenge(
  ...
  if (typeof query === 'string' || query instanceof URL) {
    const urlObj = typeof query === 'string' ? new URL(query, 'http://localhost') : query;

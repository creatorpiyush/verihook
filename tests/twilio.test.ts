...
describe("Twilio Webhook Verifier", () => {
  ...
  it("should verify relative URLs using query fallback Regex replacement", async () => {
    const relUrl = "/webhooks/twilio?bodySHA256=stale";
    const hashBytes = await computeSha256(jsonBody);
    const hashHex = bytesToHex(hashBytes).toLowerCase();

    const dataToSign = `/webhooks/twilio?bodySHA256=${encodeURIComponent(hashHex)}`;
    const hmac = await computeHmacSha1(secret, dataToSign);
    const signature = bytesToBase64(hmac);
  ...
  it("should verify relative URLs without existing query params", async () => {
    const relUrl = "/webhooks/twilio";
    const hashBytes = await computeSha256(jsonBody);
    const hashHex = bytesToHex(hashBytes).toLowerCase();

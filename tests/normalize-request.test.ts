...
import {
  ...
  normalizeHeaders,
  normalizeRequest,
} from "../src/utils/normalize-request.js";
...
describe("normalizeRequest & Utilities", () => {
  ...
  it("should handle undefined headers safely", () => {
  ...
  it("should normalize body from string, Uint8Array, ArrayBuffer, Buffer, numbers, and explicit raw object", () => {
    expect(normalizeBody("hello world")).toBe("hello world");
    expect(normalizeBody(undefined)).toBe("");
    expect(normalizeBody(null)).toBe("");
    expect(normalizeBody(12345)).toBe("12345");
    expect(normalizeBody(new TextEncoder().encode("hello bytes"))).toBe(
      "hello bytes",
    );
    const arrayBuf = new TextEncoder().encode("hello arraybuf").buffer;
    expect(normalizeBody(arrayBuf)).toBe("hello arraybuf");
  ...
  it("should throw when plain object is passed without rawBody", () => {
    expect(() => normalizeBody({ key: "value" })).toThrow(
      "Parsed object passed as request body without rawBody",
    );
  });

  it("should throw when null or undefined input is passed to normalizeRequest", async () => {
    await expect(normalizeRequest(null as any)).rejects.toThrow(
      "Request input cannot be null or undefined",
    );
  });
